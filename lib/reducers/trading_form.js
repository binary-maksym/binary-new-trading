...
class ActionsHandlers {
    ...
    static SET_SET_CATEGORY(state, action) {

        const setError = error => state.set('error', Err._(error));
        const setCategory = category => state.set('category', category).setIn(['storage', 'category'], category);
