...
function TradingFormActions(store) {
    ...
    actions.setMarket = value => {
        const dispatcher = getDispatcher(Market.getAction(value));
        getSymols().then(() => dispatcher()).catch(error => handleError(error));
    };

    actions.setSymbol = value => {
        const dispatcher = getDispatcher(Symbol1.getAction(value));
        getSymols().then(() => dispatcher()).catch(error => handleError(error));
    };
