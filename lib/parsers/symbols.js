...
export default class SymbolsParser {
    ...
    _parse(s) {
        ...
        if (markets.isEmpty()) {
            result.push(Err._('ERROR_SYMBOLS_PARSING'));
        }
        return result;
    }
    ...
    _sortSymbolsList(symbols = List()) {
        if (List.isList(symbols)) {
            symbols = symbols.sort((a, b) => {
                return Order.markets[a.get('submarket')] - Order.markets[b.get('submarket')]
            }).sort((a, b) => {
    ...
    /**
     * Returns ordered markets. Markets order is taken from /config/order.json
     *
     * @method getMarkets
    ...
    getMarkets(only_opened = 0) {
        let markets = List();
        this.tree.sort((a, b) => {
            return Order.markets[a.get('market')] - Order.markets[b.get('market')]
        }).forEach((a) => {
            const row = Map({
                market: a.get('market'),
                name: a.get('name'),
     ...
     *
     * @method getFirstOpenedMarket
     * @return {Map} Market details    
     */
    getFirstOpenedMarket() {
     ...
     *
     * @method getFirstOpenedSymbol
     * @param {String} MARKET
     * @return {Map} SYMBOL details    
     */
    getFirstOpenedSymbol(market) {
     ...
     *
     * @method isSymbolOpened
     * @param {String} SYMBOL
     * @return {Bool}     
     */
    isSymbolOpened(symbol) {
